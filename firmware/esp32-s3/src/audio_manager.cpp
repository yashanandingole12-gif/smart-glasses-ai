...
bool AudioManager::initMicrophone() {
  // 1. Configure I2S driver in Standard I2S RX mode for INMP441 Digital
  // Microphone
  i2s_config_t i2s_mic_config = {
      .mode = (i2s_mode_t)(I2S_MODE_MASTER | I2S_MODE_RX),
      .sample_rate = I2S_MIC_SAMPLE_RATE,
      .bits_per_sample = I2S_BITS_PER_SAMPLE_16BIT,
      .channel_format = I2S_CHANNEL_FMT_ONLY_LEFT, // INMP441 with L/R tied to
                                                   // GND is Left Channel
  ...
  i2s_pin_config_t mic_pin_config = {
      .bck_io_num = I2S_MIC_BCLK, // GPIO 7 (D8)
      .ws_io_num = I2S_MIC_LRCLK, // GPIO 4 (D3)
      .data_out_num = I2S_PIN_NO_CHANGE,
      .data_in_num = I2S_MIC_DATA // GPIO 9 (D10)
  };

  esp_err_t err = i2s_driver_install(I2S_MIC_PORT, &i2s_mic_config, 0, NULL);
  if (err == ESP_OK) {
    i2s_set_pin(I2S_MIC_PORT, &mic_pin_config);
    i2s_set_clk(I2S_MIC_PORT, I2S_MIC_SAMPLE_RATE, I2S_BITS_PER_SAMPLE_16BIT,
                I2S_CHANNEL_MONO);
    _micInitialized = true;
    Serial.printf("[AUDIO] INMP441 I2S Microphone Driver Active (16kHz Mono, "
                  "SCK=GPIO%d, WS=GPIO%d, SD=GPIO%d)\n",
                  I2S_MIC_BCLK, I2S_MIC_LRCLK, I2S_MIC_DATA);
  } else {
    Serial.printf("[AUDIO] INMP441 I2S Mic Driver Install Failed (err=0x%x)\n",
                  err);
...
bool AudioManager::initSpeaker() {
  ...
  i2s_config_t i2s_spk_config = {
      ...
      .bits_per_sample = I2S_BITS_PER_SAMPLE_16BIT,
      .channel_format = I2S_CHANNEL_FMT_RIGHT_LEFT, // Stereo frames ensure
                                                    // MAX98357A compatibility
      .communication_format = I2S_COMM_FORMAT_STAND_I2S,
      .intr_alloc_flags = ESP_INTR_FLAG_LEVEL1,
      .dma_buf_count = 6,
      .dma_buf_len = 256,
      .use_apll = false,
      .tx_desc_auto_clear =
          true, // Zero DMA buffer on underflow to prevent buzzing
      .fixed_mclk = 0};

  i2s_pin_config_t spk_pin_config = {.bck_io_num = I2S_SPK_BCLK, // GPIO 7 / D8
                                     .ws_io_num = I2S_SPK_LRCLK, // GPIO 8 / D9
                                     .data_out_num =
                                         I2S_SPK_DATA, // GPIO 9 / D10
  ...
  if (err == ESP_OK) {
    ...
    Serial.printf("[AUDIO] I2S Speaker Driver Active (16kHz Stereo-Out, "
                  "BCLK=%d, LRC=%d, DATA=%d)\n",
...
size_t AudioManager::readMicrophone(int16_t *buffer, size_t maxSamples) {
  ...
  size_t bytesRead = 0;
  esp_err_t err =
      i2s_read(I2S_MIC_PORT, (void *)buffer, maxSamples * sizeof(int16_t),
...
void AudioManager::runMicrophoneDiagnostic(uint32_t durationMs) {
  ...
  if (maxRMS > 150.0f || absolutePeak > 300) {
    Serial.println("  - Final Verdict: [PASS] ESP32 Microphone is active & "
                   "receiving audio!");
  } else {
...
void AudioManager::playWavAudio(const uint8_t *wavBytes, size_t wavLength) {
  ...
  Serial.printf(
      "[AUDIO] Playing WAV Audio: %u bytes (%u Hz, %d-channel, %u samples)\n",
...
void AudioManager::streamPcmChunk(const uint8_t *pcmChunk, size_t chunkSize) {
  ...
  float volScale =
      ((float)_volume / 100.0f) * 0.85f; // Headroom to prevent digital clipping
...
void AudioManager::runSpeakerDiagnostic() {
  if (!_speakerInitialized) {
    Serial.println(
        "[SPEAKER DIAGNOSTIC] ERROR: I2S Speaker driver not initialized.");
    return;
  }
...
void AudioManager::triggerVoiceWakeSession() {
  ...
  playSound(SoundEffect::SOUND_WAKE);
  startMicrophone();
  Serial.println("[VOICE WAKE] >>> WAKE TRIGGER ACTIVATED -> Sending "
                 "TALK_START to Mobile Phone");
...
void AudioManager::stopVoiceWakeSession() {
  ...
  Serial.println("[VOICE WAKE] <<< WAKE SESSION STOPPED -> Sending TALK_STOP "
                 "to Mobile Phone");
...
void AudioManager::update() {
  ...
  if (millis() - _lastDiagnosticTime >= 120) {
    ...
    if (_vadEnabled) {
      // Filter out ambient noise (< 250 RMS)
      if (rms > 250.0f) {
        _lastSpeechTime = millis();
        ...
        if (!_speechActive && _consecutiveVoiceFrames >= 2) {
          ...
          _speechStartTime = millis();
          startMicrophone();
          Serial.printf("[VOICE WAKE] >>> Speech Detected (RMS: %.0f) -> "
                        "Sending TALK_START to Mobile Phone\n",
                        rms);
          BleManager::getInstance().sendEvent("TALK_START");
        }
      } else {
      ...
      // Universal session termination: 1.2s silence OR 6s max safety timeout
      if (_speechActive && ((millis() - _lastSpeechTime > 1200) ||
                            (millis() - _speechStartTime > 6000))) {
        _speechActive = false;
        _consecutiveVoiceFrames = 0;
        stopMicrophone();
        Serial.println("[VOICE WAKE] <<< Speech Session Finished -> Sending "
                       "TALK_STOP to Mobile Phone");
        BleManager::getInstance().sendEvent("TALK_STOP");
      }
    }
  }
...
bool AudioManager::recordWavAudio(uint32_t durationMs, uint8_t **outWavBuffer,
  ...
  uint8_t *wavBuf = (uint8_t *)(psramFound() ? ps_malloc(totalWavBytes)
                                             : malloc(totalWavBytes));
  if (!wavBuf) {
    Serial.printf(
        "[AUDIO] ERROR: Failed to allocate %u bytes for WAV recording buffer\n",
  ...
  Serial.printf("[AUDIO] Recording %u ms audio from ESP32 digital mic (16kHz "
                "16-bit Mono WAV)...\n",
                (unsigned int)durationMs);

  while (samplesRecorded < totalSamples &&
         (millis() - startTime < durationMs + 500)) {
    size_t toRead = (totalSamples - samplesRecorded > 256)
                        ? 256
                        : (totalSamples - samplesRecorded);
    size_t readCount = readMicrophone(tempChunk, toRead);
    if (readCount > 0) {
      memcpy(pcmPayload + samplesRecorded, tempChunk,
             readCount * sizeof(int16_t));
  ...
  *outWavSize = totalWavBytes;
  Serial.printf(
      "[AUDIO] Audio recording complete: %u bytes (%u samples, %u ms)\n",
      (unsigned int)totalWavBytes, (unsigned int)samplesRecorded,
      (unsigned int)(samplesRecorded * 1000 / I2S_MIC_SAMPLE_RATE));
  return true;
}
